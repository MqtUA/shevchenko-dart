...
беруться із секції `# <version>` у `CHANGELOG.md`; відсутня або порожня секція
зупиняє release. Повторний запуск не створює release, якщо такий тег уже має
GitHub Release.
...
Команда виконує `dart pub publish --dry-run --ignore-warnings`, показує повний
звіт валідатора та завершується помилкою, якщо архів пакета неможливо створити.
Перед публікацією перевірте package metadata.
